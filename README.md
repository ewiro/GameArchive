...
- 展示条目信息、续集、制作人员、角色与声优、简介和观看记录，并可打开角色、人物及公司介绍。
...
- 如果关闭动漫页，只展示游戏活动记录。
- 支持从 Obsidian Markdown 笔记导入游戏标签、游玩历史和动漫观看历史。
...
### Obsidian Markdown 导入

打开“设置 → 数据管理 → 导入 Obsidian 记录”，选择保存笔记的文件夹。应用会递归读取该文件夹及其子文件夹中的 `.md` 文件。每篇笔记必须以 `---` 开始，并把待导入数据写在文件开头的属性区中。
...
- 必须使用分行列表形式的 `tags`，并包含 `游戏记录` 或 `动漫记录`；不支持 `tags: [游戏记录]` 形式。
- 可使用文件名、`中文名`、`英文名` 和 `外文名` 匹配条目。应用先严格匹配完整名称，找不到时才兼容游戏年度版后缀或动漫电影/剧场版前缀；无法唯一匹配时会跳过笔记。
- 游戏必须存在于已配置 Steam 账号的库存中。游戏笔记会导入历史和除 `游戏记录` 外的其他标签；动漫笔记只导入历史。
- `历史记录` 的日期必须写成 `YYYY-MM-DD`，数值必须大于 `0`。游戏数值单位为小时，例如 `1.4` 等于 84 分钟；动漫数值单位为集，支持小数。
- 所有导入结果只保存在设备本地，不会写入 Bangumi 云端。相同条目和日期已有记录时不会累加或覆盖，因此可以安全重复导入。
